...
class ProviderUPS(models.Model):
    ...
    def ups_get_return_label(self, picking, tracking_number=None, origin_date=None):
        ...
        if picking.is_return_picking:
            ...
            if picking.package_ids:
                ...
                for package in picking.package_ids:
                    packages.append(
                        Package(self, package.shipping_weight, quant_pack=package.package_type_id, name=package.name))
        ...
        check_value = srm.check_required_value(picking.partner_id, picking.partner_id,
        ...
        if self.ups_label_file_type != 'GIF':
            attachments = [
                ('%s-%s-%s.%s' % (self.get_return_label_prefix(), pl[0], index, self.ups_label_file_type), pl[1]) for
                index, pl in enumerate(package_labels)]
        if self.ups_label_file_type == 'GIF':
            attachments = [('%s-%s-%s.%s' % (self.get_return_label_prefix(), package_labels[0][0], 1, 'pdf'),
