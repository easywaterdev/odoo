...
class MailMessage(models.Model):
    ...
    def _message_fetch(self, domain, max_id=None, min_id=None, limit=30):
        """
        Overwrite method to check actions and search
        with archived record for trash action
        """
        trash_id = self.env.ref("prt_mail_messages.action_prt_mail_messages_trash").id
        archive_id = self.env.ref(
            "prt_mail_messages.action_prt_mail_messages_archived"
        ).id
        action_id = self._context.get("action_id", False)
        force_message_id = self._context.get("force_message_id", False)
        force_message_model = self._context.get("force_message_model", False)
        if action_id in [trash_id, archive_id]:
        ...
        if force_message_id:
            force_domain = [
                ...
                "&",
                ("id", "=", force_message_id),
                ("model", "=", force_message_model),
            ]
            if domain:
                domain = force_domain + domain
        return super(MailMessage, self)._message_fetch(
