...
//                            self.grid.addWidget($(item_view), self.gridstackConfig[items[i].id].x, self.gridstackConfig[items[i].id].y, self.gridstackConfig[items[i].id].width, self.gridstackConfig[items[i].id].height, false, 6, null, 2, 2, items[i].id);
                        ...
                        var $to_do_preview = self.ksRenderToDoDashboardView(items[i]);
                        if (items[i].id in self.gridstackConfig) {
                            self.grid.addWidget($to_do_preview[0], {x:self.gridstackConfig[items[i].id].x, y:self.gridstackConfig[items[i].id].y, w:self.gridstackConfig[items[i].id].w, h:self.gridstackConfig[items[i].id].h, autoPosition:false, minW:4, maxW:null, minH:3, maxH:null, id:items[i].id});
                        } else {
                            self.grid.addWidget($to_do_preview[0], {x:0, y:0, w:5, h:4, autoPosition:true, minW:4, maxW:null, minH:3, maxH:null, id:items[i].id})
                        }
                    } else {
