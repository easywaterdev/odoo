...
TREE_TEMPLATE = (
    ...
    "<tbody>"
    "<tr>"
    '<td style="width: 1%%;"><img class="img-circle"'
    ' style="height: auto; width: 64px; padding:10px;"'
    ' src="data:image/png;base64, %s" alt="Avatar" title="%s"'
...
class Conversation(models.Model):
    ...
    def _compute_subject_display(self):
        ...
        for rec in self:
            ...
            for participant in rec.partner_ids:
                participant_text = " ".join((
                    ...
                    ' title="%s" src="data:image/png;base64, %s"/>' % (
                        ...
                        participant.image_small.decode("utf-8")
                        if participant.image_small else IMAGE_PLACEHOLDER,
                    ),
                ))
            ...
            rec.subject_display = TREE_TEMPLATE % (
                rec.author_id.image_small.decode("utf-8")
                if rec.author_id and rec.author_id.image_small else IMAGE_PLACEHOLDER,
    ...
    def message_new(self, msg_dict, custom_values=None):
            ...
            author_id = (
                partner.id if partner else self.env["res.partner"].create({
                    ...
                    "category_id": [(
                        4,
                        self.env
                        .ref("prt_mail_messages.cetmix_conversations_partner_cat").id,
                    )] if self.env.ref(
                ...
                if not partner:
                    partner = self.env["res.partner"].create({
                        ...
                        "category_id": [(
                            4,
                            self.env.ref(
                                "prt_mail_messages.cetmix_conversations_partner_cat"  # noqa
                            ).id,
                        )] if self.env.ref(
                            "prt_mail_messages.cetmix_conversations_partner_cat"  # noqa
                        ) else False,  # noqa
        ...
        # msg_dict.update({'subtype': 'prt_mail_messages.mt_conversation_create'})
        return super(
            Conversation,
            self.with_context(
    ...
    def message_post(
        ...
        **kwargs
        ...
        if message_type not in ["comment", "email"]:  # Skip notifications
            return res
        # Add other_partner_ids as followers. We do it here
        # to avoid them being notified on initial message post
